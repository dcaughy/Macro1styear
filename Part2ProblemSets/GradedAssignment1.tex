...
\[\iff \beta=\eta(\lambda)\]
This is the same Hosios condition that we saw in class. The intuition is that the efficient solution depends on the elasticity of the matching function. In words, the bargining power of the firm must be equal to the elasticity of the matching function for the firm. 
...
Equilibrium $\lambda$ will only ever be efficient for a strictly positive bargaining power for workers, while the condition for k states that production will only ever be efficient when workers have no bargaining power. The incentives for the optimal queue length and the optimal level of production do not align. The two choices being made in the market economy are over capital and entry, while in the planner's economy the choices are for capital and queue length. The incentives for the firm do not take into account the value lost from the worker associated with a large queue length. A large queue length incentivises firms to enter by ensuring that costs of vacancies are small. Furthermore, firms, being the agent that decide capital in the market will only choose the efficient level when they are the sole beneficiaries of production. These in turn cause the market level of entry and capital to both be too small.
