...
\[\bigl(1-\eta(\lambda)\bigr)f(k)+k\bigl(\frac{\delta\eta(\lambda)}{m(\lambda)}+\frac{1}{\lambda}\bigr)\]
...
Now taking the First order condition for $\lambda$ and solving for $f(k)$:
\[f(k)=-\frac{1}{1-\eta(\lambda)}k(\frac{\delta \eta(\lambda)}{m(\lambda)} +\frac{1}{\lambda}\bigr)\]
Then if the two solutions are equal we have
\[\frac{1}{1-\beta} k \bigl(\frac{\beta}{\lambda}+\frac{\delta}{m(\lambda)}\bigr)=-\frac{1}{1-\eta(\lambda)}k(\frac{\delta \eta(\lambda)}{m(\lambda)} +\frac{1}{\lambda}\bigr)\]
\[\iff (1-\eta(\lambda))\frac{\beta m(\lambda)+\lambda \delta}{\lambda m(\lambda)}=(1-\beta)\frac{\lambda \delta \eta(\lambda)+m(\lambda)}{\lambda m(\lambda)}\]
\[\iff \beta m(\lambda)+\lambda \delta-\bigl(\lambda \delta \eta(\lambda)+m(\lambda)\bigr)=\eta(\lambda)\beta m(\lambda)+\lambda\delta \eta(\lambda)-\lambda\delta \beta \eta(\lambda)-\beta m(\lambda)\]
\[\iff \beta\bigl(2m(\lambda)-\eta(\lambda)m(\lambda)+\lambda\delta\eta(\lambda)\bigr)=2\lambda\delta\eta(\lambda)-\lambda\delta+m(\lambda)\]
\[\iff \beta=\frac{2\lambda\delta\eta(\lambda)-\lambda\delta+m(\lambda)}{2m(\lambda)-\eta(\lambda)m(\lambda)+\lambda\delta\eta(\lambda)}\]
...
From the two efficiency conditions derived from parts d) and e) together imply that this economy will never be efficient. The two conditions on $\beta$ are disjoint. Equilibrium $\lambda$ will only ever be efficient for a strictly positive bargaining power for workers, while the condition for k states that production will only ever be efficient when workers have no bargaining power. 
